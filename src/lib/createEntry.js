...
const createEntry = (request, title, content, options, cookie) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (_options.saveAsDraft) {
      ...
      axios.post(`${URLS.BASE}/${actualPath}/savedraft`, qs.stringify(requestBody), config)
        .then((res) => {
          if (res.data.Success) {
      ...
      axios.get(`${URLS.BASE}/?q=${title}`, {
        ...
        const entryIdRegex = new RegExp('(?<=li class="hidden" data-id=")(.*)(?=" data-author=")', 'u')
        const entryId = entryIdRegex.exec(res.data)[0]
        resolve(new EntryForMember(request, entryId, cookie))
      }).catch((err) => {
        // detect if entry already exist
        if (err.response && err.response.status === 404 && err.response.data.includes('böyle bir entry zaten var')) {
